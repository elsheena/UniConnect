...
namespace Admin.BLL.Services
{
    public class EmailService : IEmailService
    {
        ...
        public EmailService(IConfiguration config, AdminDbContext db)
        {
            _config = config;
            _db = db;
        }
        ...
        public async Task SendApplicationEmailAsync(string applicantName, string applicantEmail, string applicantNationality, string universityName, string programName, string programCode, List<(string FilePath, string OriginalName)> attachments)
        {
            try
            {
                ...
                Console.WriteLine($"[EmailService] Queued background application email for applicant {applicantName}");
            }
            catch (Exception ex)
            {
                Console.WriteLine("[EmailService] Error: Failed to queue email: " + ex.Message);
            }
        }
    }
}
